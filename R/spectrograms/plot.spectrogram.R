plot.spectrogram = function (x, y, ylim,xlim, quality = FALSE, ...){
  ...
  times = as.numeric(rownames (x$spectrogram))/1000
  ...
  if (!quality){
    image (as.double(times), as.double(hz), x$spectrogram, useRaster = FALSE, col = zcolors, xlab = 'Time (s)', ylab = "Frequency (Hz)", ylim = ylim, xlim = xlim,...)
  }
  box()
}
